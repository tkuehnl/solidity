...
- If the error was caused by ``revert("reasonString")`` or
...
- If the error was caused by a panic, i.e. by a failing ``assert``, division by zero,
  invalid array access and others, the catch clause of the type
  ``catch Panic(uint errorCode)`` will be run.
...
The strings ``Error`` and ``Panic`` are currently parsed as is and is not treated as an identifier.
